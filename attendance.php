...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'clock_in') {
        ...
        // Face check only required if NOT upload mode
        if (!$uploadMode) {
        ...
        $conn->query("INSERT INTO attendance (user_id, type, clock_in, date, photo_in, status, punctuality, minutes_late, upload_mode)
                      VALUES ($uid, 'user', '$now', '$today', '$photoEsc', 'present', '$pStatus', $minsLate, $uploadMode)");
...
.cam-mode-tab{flex:1;padding:9px 10px;border:none;background:transparent;font-family:'Jost',sans-serif;font-size:11.5px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;color:var(--muted);cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:6px}
.cam-mode-tab.active{background:var(--card);color:var(--gold);box-shadow:inset 0 -2px 0 var(--gold)}

/* UPLOAD ZONE */
.upload-zone{border:2px dashed var(--border);border-radius:4px;padding:32px 20px;text-align:center;cursor:pointer;transition:all 0.2s;background:var(--surface);position:relative;aspect-ratio:4/3;display:flex;align-items:center;justify-content:center}
.upload-zone:hover{border-color:var(--gold-dim);background:rgba(201,168,76,0.04)}
...
.upload-mode-note{background:rgba(212,130,10,0.06);border:1px solid rgba(212,130,10,0.2);border-radius:3px;padding:10px 14px;font-size:11.5px;color:var(--muted);line-height:1.6;margin-top:10px}
...
<?php elseif ($todayRecord && ($todayRecord['punctuality'] ?? '') === 'late'): ?>
    <div class="notif-card danger">
        ...
        <div class="notif-text">
            ...
            <?php if (!empty($todayRecord['upload_mode'])): ?><span class="upload-badge">Photo Upload</span><?php endif; ?>
        </div>
    </div>
    ...
    <div id="tabMyContent">
        ...
        <div class="status-row" style="margin-bottom:24px">
            <div class="status-card">
                ...
                <?php endif; ?>
                <?php if (!empty($todayRecord['upload_mode'])): ?>
                <div style="margin-top:6px"><span class="upload-badge">Photo Upload Mode</span></div>
                <?php endif; ?>
            </div>
            <div class="status-card">
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    <div class="cam-mode-tabs">
                        <button class="cam-mode-tab active" id="mode-cam-btn" onclick="switchCamMode('camera')">
                        ...
                        <button class="cam-mode-tab" id="mode-upload-btn" onclick="switchCamMode('upload')">
                        ...
                        <div class="upload-zone" id="upload-zone">
                            ...
                            <div class="upload-zone-inner" id="upload-zone-inner">
                                ...
                                <div class="upload-zone-text">
                                    <strong>Tap to upload your photo</strong>
                                    It requires images or any clear photo.<br>
                                    <span style="font-size:11px;opacity:0.7">JPG, PNG, WEBP — max 5MB</span>
                                </div>
                            </div>
                            <img id="upload-preview" alt="Upload preview">
                        </div>
                        <div class="upload-mode-note">
                            <strong>⚠ Manual Reference Mode</strong> — There is no face recognition here. The uploaded photo will serve as attendance proof that will be reviewed by the admin.
                        </div>
                        <div class="cam-btns">
                            <button class="btn btn-muted" id="upload-retake-btn" style="display:none" onclick="clearUpload()">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.54"/></svg>
                                Change the photo
                            </button>
                        </div>
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    <div style="background:rgba(201,168,76,0.06);border:1px solid var(--gold-dim);border-radius:3px;padding:12px 14px;font-size:12.5px;color:var(--muted);margin-bottom:16px;line-height:1.6">
                        Para sa Camera mode, kailangan munang i-register ang mukha mo.
                    ...
                    <div class="action-btns">
                        ...
                        ?>
                        <button class="clock-btn clock-in-btn" id="clock-in-btn" <?= !$canClockIn ? 'disabled' : '' ?>>
        ...
        <div>
            ...
            <div class="table-card">
                <table>
                    <thead><tr><th>Date</th><th>Clock In</th><th>Punctuality</th><th>Clock Out</th><th>Hours</th><th>Mode</th><th>Status</th></tr></thead>
                    <tbody>
                    <?php if (empty($history)): ?>
                        <tr class="empty-row"><td colspan="7">No attendance records yet.</td></tr>
                    ...
                    ?><tr>
                        ...
                        <td>
                            <?php if ($isUpload): ?>
                                <span class="upload-badge">Photo Upload</span>
                            <?php else: ?>
                                <span style="font-size:11px;color:var(--muted)">Camera</span>
                            <?php endif; ?>
                        </td>
    ...
    <div id="tabAdminContent" style="display:none">
        ...
        <div>
            ...
            <div class="table-card" style="margin-bottom:24px">
                <table>
                    <thead><tr><th>Staff</th><th>Clock In</th><th>Punctuality</th><th>Clock Out</th><th>Hours</th><th>Mode</th><th>Status</th></tr></thead>
                    <tbody>
                    <?php if (empty($adminStats)): ?>
                        <tr class="empty-row"><td colspan="7">No staff have clocked in today yet.</td></tr>
        ...
        <div>
            ...
            <div class="table-card">
                <table>
                    <thead><tr><th>Date</th><th>Staff</th><th>Clock In</th><th>Punctuality</th><th>Clock Out</th><th>Hours</th><th>Mode</th></tr></thead>
                    <tbody>
                    <?php if (empty($adminHistory)): ?>
                        <tr class="empty-row"><td colspan="7">No records in the last 7 days.</td></tr>
...
<div id="faceModal">
    <div class="faceModal-card">
        ...
        <div class="modal-sub">I-scan ang mukha mo gamit ang camera o mag-upload ng malinaw na photo. Gagamitin ito para sa clock in/out verification.</div>

        <!-- REG MODE TOGGLE -->
        <div class="cam-mode-tabs" style="margin-bottom:14px">
            <button class="cam-mode-tab active" id="reg-mode-cam-btn" onclick="switchRegMode('camera')">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
                Camera
            </button>
            <button class="cam-mode-tab" id="reg-mode-upload-btn" onclick="switchRegMode('upload')">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                Upload Image
            </button>
        </div>
        ...
        <!-- UPLOAD SECTION -->
        <div id="reg-upload-section" style="display:none">
            <div class="upload-zone" id="reg-upload-zone">
                <input type="file" id="reg-upload-input" accept="image/*" onchange="handleRegImageUpload(this)">
                <div class="upload-zone-inner" id="reg-upload-zone-inner">
                    <div class="upload-zone-icon">
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                    </div>
                    <div class="upload-zone-text">
                        <strong>Mag-upload ng malinaw na photo ng mukha mo</strong>
                        Selfie, ID photo, o anumang malinaw na litrato<br>
                        <span style="font-size:11px;opacity:0.7">JPG, PNG — max 5MB</span>
                    </div>
                </div>
                <img id="reg-upload-preview" alt="Uploaded face preview">
            </div>
            <div id="reg-upload-status" style="font-size:12.5px;color:var(--muted);margin-top:10px;min-height:20px"></div>
            <canvas id="reg-upload-canvas" style="display:none"></canvas>
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
                <button class="btn btn-gold" id="reg-upload-scan-btn" disabled>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                    Scan Face from Image
                </button>
                <button class="btn btn-gold" id="reg-upload-save-btn" disabled>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
                    Save & Register
                </button>
                <button class="btn btn-muted" id="reg-upload-retake-btn" style="display:none" onclick="clearRegUpload()">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.54"/></svg>
                    Palitan
                </button>
            </div>
...
function switchCamMode(mode) {
    ...
    if (!isCamera) {
        // Switching to upload: stop camera stream if running
...
function handleImageUpload(input) {
    ...
    if (file.size > 5 * 1024 * 1024) {
        showToast('File Too Large', 'Maximum 5MB lang ang pwedeng i-upload.', 'error');
        input.value = '';
        return;
    }
    ...
    reader.onload = function(e) {
        ...
        document.getElementById('upload-face-status').innerHTML =
            '<div class="fs-ok"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg> Photo ready — proceed to clock in/out below</div>';
...
function clearUpload() {
    ...
    const retakeBtn = document.getElementById('upload-retake-btn');
    if (retakeBtn) retakeBtn.style.display = 'none';
    const statusEl = document.getElementById('upload-face-status');
    if (statusEl) statusEl.innerHTML = '';
    const inputEl = document.getElementById('att-upload-input');
    if (inputEl) inputEl.value = '';
}

function isUploadMode() {
    const el = document.getElementById('upload-mode-section');
    return el && el.style.display !== 'none';
}

function getPhotoForSubmit() {
    return isUploadMode() ? uploadState.photo : (state.capturedPhoto || null);
}

function isReadyForClock() {
    if (isUploadMode()) return uploadState.ready;
    return state.captured && state.faceMatched;
}

// ── BROWSER NOTIFICATIONS ───────────────────────────────────────────────────
function checkNotifPermission() {
    if (!('Notification' in window)) return;
    if (Notification.permission === 'default') document.getElementById('notifPermStrip').style.display = 'flex';
}
...
function scheduleShiftReminders() {
    ...
    if (ms30 > 0) setTimeout(() => new Notification('☕ AyosCoffeeNegosyo', { body: 'Your shift starts in 30 minutes (8:00 AM). Prepare to clock in!', icon: '/favicon.ico' }), ms30);
    const ms10 = shift - now - 10*60*1000;
    if (ms10 > 0) setTimeout(() => new Notification('⏰ Almost Time!', { body: 'Your shift starts in 10 minutes. Head to the attendance page now!', icon: '/favicon.ico' }), ms10);
    const msShift = shift - now;
    if (msShift > 0) setTimeout(() => new Notification('🔔 Shift Started!', { body: 'It is now 8:00 AM. Clock in now to be marked On Time!', icon: '/favicon.ico' }), msShift);
    const msLate = shift - now + 5*60*1000;
    if (msLate > 0) setTimeout(() => new Notification('🔴 You May Be Late!', { body: 'It is 8:05 AM and you have not clocked in. Clock in now!', icon: '/favicon.ico' }), msLate);
}
    ...
    try {
        ...
        if (faceRegistered && storedDescriptor) {
            ...
            if (dist < FACE_THRESHOLD) {
                ...
                faceStatus.innerHTML = '<div class="fs-fail"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg> Face not recognized. Please retake or use Upload Image.</div>';
            }
...
document.getElementById('clock-in-btn').addEventListener('click', async function() {
    if (!isReadyForClock()) {
        const msg = isUploadMode()
            ? 'Mag-upload muna ng photo mo bago mag-clock in.'
            : 'Capture and verify your face first.';
    ...
    fd.append('photo', getPhotoForSubmit());
    ...
    try {
        ...
        if (data.success) {
            ...
            if (Notification.permission === 'granted') new Notification(title, { body: data.message, icon: '/favicon.ico' });
...
document.getElementById('clock-out-btn').addEventListener('click', async function() {
    if (!isReadyForClock()) {
        const msg = isUploadMode()
            ? 'Mag-upload muna ng photo mo bago mag-clock out.'
            : 'Capture and verify your face first.';
        showToast('Verification Required', msg, 'error');
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            if (Notification.permission === 'granted') new Notification('Clocked Out', { body: 'You worked ' + data.duration + ' today. See you tomorrow!', icon: '/favicon.ico' });
            setTimeout(() => location.reload(), 2000);
        } else {
...
let regUploadState = { imageEl: null, descriptor: null, ready: false };

async function openFaceModal() {
    document.getElementById('faceModal').classList.add('show');
    // Default to camera mode and start it
    switchRegMode('camera');
}

function switchRegMode(mode) {
    const isCamera = mode === 'camera';
    document.getElementById('reg-camera-section').style.display = isCamera ? '' : 'none';
    document.getElementById('reg-upload-section').style.display = isCamera ? 'none' : '';
    document.getElementById('reg-mode-cam-btn').classList.toggle('active', isCamera);
    document.getElementById('reg-mode-upload-btn').classList.toggle('active', !isCamera);

    if (isCamera) {
        // Stop upload state, start camera
        regUploadState = { imageEl: null, descriptor: null, ready: false };
        startRegCamera();
    } else {
        // Stop camera stream if running
        if (regState.stream) { regState.stream.getTracks().forEach(t => t.stop()); regState.stream = null; }
        regState.cameraReady = false;
        // Reset camera UI
...
async function startRegCamera() {
    ...
    if (!regState.stream) {
        document.getElementById('reg-status').textContent = 'Camera access denied. Try Upload Image tab instead.';
        document.getElementById('reg-status').style.color = '#e05a5a';
        return;
    }
...
function closeFaceModal() {
    ...
    document.getElementById('reg-retake-btn').disabled = true;
    document.getElementById('reg-status').textContent = '';
    // Reset upload section too
    clearRegUpload();
    // Switch back to camera tab visually
    document.getElementById('reg-camera-section').style.display = '';
    document.getElementById('reg-upload-section').style.display = 'none';
    document.getElementById('reg-mode-cam-btn').classList.add('active');
    document.getElementById('reg-mode-upload-btn').classList.remove('active');
}
...
document.getElementById('reg-save-btn').addEventListener('click', async function() {
    ...
    await saveRegDescriptor(this, regState.capturedDescriptor);
});

// ── UPLOAD IMAGE REGISTRATION ────────────────────────────────────────────────
function handleRegImageUpload(input) {
    const file = input.files[0];
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { showToast('File Too Large', 'Max 5MB lang ang allowed.', 'error'); input.value = ''; return; }

    const reader = new FileReader();
    reader.onload = function(e) {
        const img = new Image();
        img.onload = function() {
            regUploadState.imageEl = img;
            regUploadState.descriptor = null;
            regUploadState.ready = false;

            document.getElementById('reg-upload-preview').src = e.target.result;
            document.getElementById('reg-upload-preview').style.display = 'block';
            document.getElementById('reg-upload-zone-inner').style.display = 'none';
            document.getElementById('reg-upload-zone').classList.add('has-image');
            document.getElementById('reg-upload-retake-btn').style.display = '';
            document.getElementById('reg-upload-scan-btn').disabled = false;
            document.getElementById('reg-upload-save-btn').disabled = true;
            document.getElementById('reg-upload-status').textContent = 'Photo loaded — tap Scan Face to extract face data.';
            document.getElementById('reg-upload-status').style.color = 'var(--gold)';
        };
...
function clearRegUpload() {
    regUploadState = { imageEl: null, descriptor: null, ready: false };
    const preview = document.getElementById('reg-upload-preview');
    if (preview) { preview.style.display = 'none'; preview.src = ''; }
    const inner = document.getElementById('reg-upload-zone-inner');
    if (inner) inner.style.display = '';
    const zone = document.getElementById('reg-upload-zone');
    if (zone) zone.classList.remove('has-image');
    const retakeBtn = document.getElementById('reg-upload-retake-btn');
    if (retakeBtn) retakeBtn.style.display = 'none';
    document.getElementById('reg-upload-scan-btn').disabled = true;
    document.getElementById('reg-upload-save-btn').disabled = true;
    const statusEl = document.getElementById('reg-upload-status');
    if (statusEl) { statusEl.textContent = ''; }
    const inputEl = document.getElementById('reg-upload-input');
    if (inputEl) inputEl.value = '';
}

document.getElementById('reg-upload-scan-btn').addEventListener('click', async function() {
    if (!regUploadState.imageEl) { showToast('Walang Image', 'Mag-upload muna ng photo.', 'error'); return; }
    if (!state.modelsLoaded) { showToast('Models Not Ready', 'Hinihintay pa ang face detection models.', 'warn'); return; }

    this.disabled = true;
    this.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 0.7s linear infinite"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.54"/></svg> Scanning...';
    document.getElementById('reg-upload-status').textContent = 'Siniscan ang mukha sa image...';
    document.getElementById('reg-upload-status').style.color = 'var(--gold)';

    try {
        // Draw image to canvas so face-api can process it
        const canvas = document.getElementById('reg-upload-canvas');
        canvas.width = regUploadState.imageEl.naturalWidth;
        canvas.height = regUploadState.imageEl.naturalHeight;
        canvas.getContext('2d').drawImage(regUploadState.imageEl, 0, 0);
        ...
        if (!full || !full.descriptor) {
            document.getElementById('reg-upload-status').innerHTML = '<span style="color:#e05a5a">Hindi mahanap ang mukha sa photo. Subukan ang ibang mas malinaw na litrato.</span>';
            this.disabled = false;
            this.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> Scan Face from Image';
            return;
        }

        regUploadState.descriptor = Array.from(full.descriptor);
        regUploadState.ready = true;

        document.getElementById('reg-upload-status').innerHTML = '<span style="color:var(--green-lt)">✓ Mukha na-detect at na-scan — i-save na para i-register!</span>';
        document.getElementById('reg-upload-save-btn').disabled = false;
        this.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> Scan Face from Image';

    } catch(e) {
        document.getElementById('reg-upload-status').innerHTML = '<span style="color:#e05a5a">Error sa pag-scan: ' + e.message + '</span>';
        this.disabled = false;
        this.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> Scan Face from Image';
    }
});

document.getElementById('reg-upload-save-btn').addEventListener('click', async function() {
    if (!regUploadState.descriptor) { showToast('No Face Data', 'I-scan muna ang mukha sa image.', 'error'); return; }
    await saveRegDescriptor(this, regUploadState.descriptor);
});

// ── SHARED SAVE FUNCTION ─────────────────────────────────────────────────────
async function saveRegDescriptor(btn, descriptor) {
    const origHTML = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation:spin 0.7s linear infinite"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.54"/></svg> Saving...';
    const fd = new FormData();
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showToast('Face Registered!', 'Maari ka nang mag-clock in gamit ang face recognition.', 'success');
            closeFaceModal();
