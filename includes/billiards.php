...
$tablesBySuite = [];
$sr = $conn->query("SELECT * FROM services WHERE active = 1 AND type = 'billiards' ORDER BY suite_id ASC, id ASC");
while ($row = $sr->fetch_assoc()) $tablesBySuite[$row['suite_id']][] = $row;
...
function render_table_card($s, $tier, $suiteName) {
    $price = svcPrice($s, $tier);
    ob_start(); ?>
    <div class="svc-card">
        <div class="svc-media">
            <img src="<?= htmlspecialchars($s['image_url']) ?>" alt="<?= htmlspecialchars($s['name']) ?>" loading="lazy">
            <span class="svc-corner">Billiards</span>
        </div>
        <div class="svc-body">
            <h4><?= htmlspecialchars($s['name']) ?></h4>
            <p><?= htmlspecialchars($s['description']) ?></p>
            <div class="svc-price">₱<?= number_format($price) ?> <span>/ hour<?= $tier === 'vip' ? ' · VIP' : '' ?></span></div>
            <div class="svc-order" data-id="<?= $s['id'] ?>" data-name="<?= htmlspecialchars($s['name']) ?>"
                 data-suite="<?= htmlspecialchars($suiteName) ?>" data-price="<?= $price ?>" data-unit="per_hour">
                <div class="form-row">
                    <div class="field"><label>Date</label><input type="date" class="svc-date" min="<?= date('Y-m-d') ?>"></div>
                    <div class="field"><label>Start Time</label><input type="time" class="svc-time"></div>
                </div>
                <div class="form-row">
                    <div class="field">
                        <label>Hours</label>
                ...
                <button type="button" class="btn-gold svc-add">Reserve Table</button>   
            </div>
        </div>
    </div>
...
<title>Billiards — Nocturne Manila Bay</title>
...
@media (prefers-reduced-motion: reduce){
    ...
    .svc-card,.page-hero,#suiteTabs{opacity:1 !important;transform:none !important;transition:none !important}
}
</style>
</head>
<body>
...
<div class="h-loader" id="hLoader" aria-hidden="true">
    ...
    <div class="h-loader-word">Billiards</div>
</div>
...
<section class="page-hero">
    <div class="sec-eyebrow"><a href="hotel-services.php" style="color:var(--h-gold-dim);text-decoration:none">Spa, Shop &amp; Entertainment</a> / <a href="entertainment.php" style="color:var(--h-gold-dim);text-decoration:none">Entertainment</a> / Billiards</div>
    <h1>Billiards <em>Lounge</em></h1>
    <p>Reserve a table by the hour — each residence keeps its own tables and rates.</p>
</section>
...
<div class="tab-panel <?= $slug === $wantedSlug ? 'active' : '' ?>" data-panel="<?= $slug ?>">
    <div class="svc-grid">
        <?php foreach (($tablesBySuite[$sid] ?? []) as $s) echo render_table_card($s, $tier, $suiteRow['name']); ?>
    </div>
</div>
<?php endforeach; ?>
</div>
...
<div class="cart-drawer" id="cartDrawer">
    ...
    <div class="cart-foot">
        ...
        <p style="font-size:12px;color:var(--h-muted);margin:12px 0">
            Please <a href="register.php?next=billiards.php" style="color:var(--h-gold)">create an account</a>
            or <a href="hotel-login.php?next=billiards.php" style="color:var(--h-gold)">log in</a> to complete your order.
        </p>
...
function renderCart() {
    ...
    list.innerHTML = cart.map((it, i) => {
        ...
        return `<div class="cart-item">
            <div><strong>${it.name}</strong><br><span style="color:var(--h-muted);font-size:11px">${it.suite} · ${it.qty} hr(s)${it.date ? ' · ' + it.date + ' ' + (it.time || '') : ''}</span></div>
...
document.querySelectorAll('.svc-order').forEach(box => {
    ...
    box.querySelector('.svc-add').addEventListener('click', () => {
        ...
        cart.push({
            ...
            date: dateEl ? dateEl.value : null, time: timeEl ? timeEl.value : null,
        });
        renderCart();
...
(function() {
    ...
    if (reduced) finish();
        ...
        window.addEventListener('load', function(){ setTimeout(finish, 450); }); setTimeout(finish, 2200); }
