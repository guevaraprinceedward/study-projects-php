...
$snacksBySuite = [];
$sr = $conn->query("SELECT * FROM services WHERE active = 1 AND type = 'snacks' ORDER BY suite_id ASC, id ASC");
while ($row = $sr->fetch_assoc()) $snacksBySuite[$row['suite_id']][] = $row;
...
function render_snack_card($s, $tier, $suiteName) {
...
<title>Local Snacks — Nocturne Manila Bay</title>
...
<section class="page-hero">
    <div class="sec-eyebrow"><a href="hotel-services.php" style="color:var(--h-gold-dim);text-decoration:none">Spa, Shop &amp; Entertainment</a> / Local Snacks</div>
    <h1>Local <em>Snacks</em></h1>
    <p>Sandwiches and drinks, delivered to your suite — every residence keeps its own small menu.</p>
</section>
...
<div class="tab-panel <?= $slug === $wantedSlug ? 'active' : '' ?>" data-panel="<?= $slug ?>">
    <div class="svc-grid">
        <?php foreach (($snacksBySuite[$sid] ?? []) as $s) echo render_snack_card($s, $tier, $suiteRow['name']); ?>
    </div>
</div>
<?php endforeach; ?>
</div>
...
<div class="cart-drawer" id="cartDrawer">
    ...
    <div class="cart-foot">
        ...
        <p style="font-size:12px;color:var(--h-muted);margin:12px 0">
            Please <a href="register.php?next=snacks.php" style="color:var(--h-gold)">create an account</a>
            or <a href="hotel-login.php?next=snacks.php" style="color:var(--h-gold)">log in</a> to complete your order.
        </p>
