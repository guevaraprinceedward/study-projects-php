...
$gamesBySuite = [];
$sr = $conn->query("SELECT * FROM services WHERE active = 1 AND type = 'casino' ORDER BY suite_id ASC, id ASC");
while ($row = $sr->fetch_assoc()) $gamesBySuite[$row['suite_id']][] = $row;
...
function render_game_card($s, $tier, $suiteName) {
                ...
                <div class="form-row">
                    <div class="field">
                        ...
                        <div class="qty-control">
                            ...
                            <input type="number" class="svc-qty" value="1" min="1" max="6" readonly>
                            <button type="button" class="qty-plus">+</button>
                        </div>
                    </div>
                    <button type="button" class="btn-gold svc-add">Reserve</button>
                </div>
            </div>
        </div>
    </div>
...
<title>Casino — Nocturne Manila Bay</title>
...
<section class="page-hero">
    <div class="sec-eyebrow"><a href="hotel-services.php" style="color:var(--h-gold-dim);text-decoration:none">Spa, Shop &amp; Entertainment</a> / <a href="entertainment.php" style="color:var(--h-gold-dim);text-decoration:none">Entertainment</a> / Casino</div>
    <h1>Casino <em>Games</em></h1>
    <p>Blackjack, poker, baccarat, roulette, and slots — each with its own session rate.</p>
</section>
...
<div class="tab-panel <?= $slug === $wantedSlug ? 'active' : '' ?>" data-panel="<?= $slug ?>">
    <div class="svc-grid">
        <?php foreach (($gamesBySuite[$sid] ?? []) as $s) echo render_game_card($s, $tier, $suiteRow['name']); ?>
    </div>
</div>
<?php endforeach; ?>
</div>
...
<div class="cart-drawer" id="cartDrawer">
    ...
    <div class="cart-foot">
        ...
        <p style="font-size:12px;color:var(--h-muted);margin:12px 0">
            Please <a href="register.php?next=casino.php" style="color:var(--h-gold)">create an account</a>
            or <a href="hotel-login.php?next=casino.php" style="color:var(--h-gold)">log in</a> to complete your order.
        </p>
...
function renderCart() {
    ...
    list.innerHTML = cart.map((it, i) => {
        ...
        return `<div class="cart-item">
            <div><strong>${it.name}</strong><br><span style="color:var(--h-muted);font-size:11px">${it.suite} · ${it.qty} session(s)${it.date ? ' · ' + it.date + ' ' + (it.time || '') : ''}</span></div>
